...
unsigned char  shiftRegisters[Number_of_Registers] = {0};
...
{
    HAL_GPIO_WritePin(LATCH_CLK_GPIO_Port, LATCH_CLK_Pin, (GPIO_PinState)state);
}

void ClockPinSet(PinState595 state)
{
    HAL_GPIO_WritePin(SERIAL_CLK_GPIO_Port, SERIAL_CLK_Pin, (GPIO_PinState)state);
}

void SerialDataPinSet(PinState595 state)
{
    HAL_GPIO_WritePin(SERIAL_DATA_GPIO_Port, SERIAL_DATA_Pin, (GPIO_PinState)state);
}
