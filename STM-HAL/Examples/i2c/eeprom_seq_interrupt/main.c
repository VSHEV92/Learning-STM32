...
void main() {
    ...
    while (1) {
	    ...
	    HAL_I2C_Master_Seq_Transmit_IT(&EXAMPLE_I2C, EEPROM_ADDR, tx_data, 1, I2C_FIRST_FRAME);

	    while(!tx_completed){}
	    tx_completed = 0;

	    // read counter value
	    HAL_I2C_Master_Seq_Receive_IT(&EXAMPLE_I2C, EEPROM_ADDR, &counter, 1, I2C_OTHER_AND_LAST_FRAME);
